...
class TaskManager:
    ...
    async def update_task_step(self, task_id: str, step: int, result: str, step_type: str = "step"):
        print(f"[Update Step] step: {step}, type: {step_type}, result: {result}")  # 添加打印语句
...
async def create_task(prompt: str = Body(..., embed=True)):
    print(f"Received prompt: {prompt}")  # 打印传入的 prompt
    task = task_manager.create_task(prompt)
    translated_prompt = translate_text(prompt, target_lang="zh-CN")  # 传递正确的 prompt
    print(f"Translated prompt: {translated_prompt}")  # 打印翻译后的文本
    return {
        "task_id": task.id,
        "translated_prompt": translated_prompt  # 👈 返回翻译内容
    }

from app.agent.trans_manus import Manus
