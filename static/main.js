...
function createTask() {
    ...
    fetch('/tasks', {
        ...
        body: JSON.stringify({ translated_prompt })
    })
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            translatedContainer.className = 'translated-result';
            translatedContainer.innerHTML = `<strong>翻译结果：</strong> ${data.translated_prompt}`;
            document.getElementById('task-container').appendChild(translatedContainer);
